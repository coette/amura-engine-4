...
import { buildWristFrame, imageSpaceLandmarks } from "./wrist-frame.js?v=e4r1.5";
import { FilesetResolver, HandLandmarker } from "./vendor/mediapipe/vision_bundle.mjs";

const REVISION = "R1.5";
const MODEL_URL = "./models/A1-Irontide-AR-pretty-mobile.glb?v=e4r1.5";
const MODEL_CONFIG_URL = "./models/A1-Irontide-AR-pretty-mobile.json?v=e4r1.5";
const WRIST_LENGTH_MM = 180;
...
let overlay=null,renderer=null,scene=null,camera=null;
let wristRig=null,assembly=null,watchAnchor=null,watchModel=null,wristMesh=null,wristMaterial=null,virtualAxis=null;
let modelPromise=null,modelConfig=DEFAULT_MODEL_CONFIG,wristMode=1,appliedMode=-1,lastSignature="",timer=0,lastGeometry=null;
let bankLandmarker=null,bankLandmarkerPromise=null;
const bankFrameCache=new Map(),bankFramePending=new Map();
const watchBasePosition=new Vector3();

const tmpX=new Vector3(),tmpY=new Vector3(),tmpZ=new Vector3(),zAxis=new Vector3(0,0,1);
const qScreen=new Quaternion(),orientationMatrix=new Matrix4();
...
function labState(){
  const s=window.AmuraEngine4AxisLab?.state||{};
  return {
    wristYmm:Number(s.wristYmm)||62,
    wristZmm:Number(s.wristZmm)||44,
    offsetYmm:Number(s.offsetYmm)||0,
    offsetZmm:Number(s.offsetZmm)||0,
    watchState:Number.isFinite(Number(s.watchState))?Number(s.watchState):0
  };
}
...
function readOrangeAxis(canvas){
  ...
  const pr=pts.map(p=>(p.x-mean.x)*d.x+(p.y-mean.y)*d.y),lo=quantile(pr,.08),hi=quantile(pr,.92);
...
function measureCloudWidth(axis){
  const img=axis.img,normal={x:-axis.elbowDir.y,y:axis.elbowDir.x},cross=[],band=clamp(axis.span*.11,8,18);
  for(let y=0;y<img.height;y+=2)for(let x=0;x<img.width;x+=2){
    const dx=x+.5-axis.midpoint.x,dy=y+.5-axis.midpoint.y,along=dx*axis.elbowDir.x+dy*axis.elbowDir.y;
...
function projectP0(axis,target){
  const p=P0_BY_TARGET.get(target)||[axis.midpoint.x,axis.midpoint.y],dx=p[0]-axis.midpoint.x,dy=p[1]-axis.midpoint.y,t=dx*axis.elbowDir.x+dy*axis.elbowDir.y;
...
async function getBankLandmarker(){
  if(bankLandmarker)return bankLandmarker;if(bankLandmarkerPromise)return bankLandmarkerPromise;
  bankLandmarkerPromise=(async()=>{
    ...
    try{bankLandmarker=await HandLandmarker.createFromOptions(files,opts("GPU"));}catch(_){bankLandmarker=await HandLandmarker.createFromOptions(files,opts("CPU"));}
...
function requestBankFrame(bank,target){
  ...
  const promise=(async()=>{
    const lm=await getBankLandmarker(),mp=lm.detect(copy),landmarks=mp?.landmarks?.[0];
    if(!landmarks||landmarks.length<18)throw new Error("MediaPipe IMAGE sin mano");
    const points=imageSpaceLandmarks(landmarks,copy.width,copy.height),frame=buildWristFrame(points,physicalHand(mp));
    if(!frame?.xAxis||!frame?.yAxis||!frame?.zAxis)throw new Error("Marco IMAGE 3D inválido");
    bankFrameCache.set(target,frame);
  })().catch(e=>{console.warn("R1.5 banco orientación",e);bankFrameCache.set(target,null);}).finally(()=>bankFramePending.delete(target));
...
function orientationFromBankFrame(axis,frame){
  ...
  const projected=Math.hypot(tmpX.x,tmpX.y);if(!Number.isFinite(projected)||projected<.06)return null;
  const currentAngle=Math.atan2(tmpX.y,tmpX.x),desiredAngle=axisHandAngle(axis),delta=wrapAngle(desiredAngle-currentAngle);
  qScreen.setFromAxisAngle(zAxis,delta);tmpX.applyQuaternion(qScreen).normalize();tmpY.applyQuaternion(qScreen).normalize();tmpZ.applyQuaternion(qScreen).normalize();
...
function fallbackOrientation(axis,target){
  const x=new Vector3(-axis.elbowDir.x,axis.elbowDir.y,0).normalize(),z=new Vector3(0,0,1),y=new Vector3().crossVectors(z,x).normalize();
  const rollQ=new Quaternion().setFromAxisAngle(x,(target||0)*Math.PI/180);y.applyQuaternion(rollQ).normalize();z.applyQuaternion(rollQ).normalize();
...
function applyLabState(){
  if(!assembly||!wristMesh)return labState();
  const s=labState();
  wristMesh.scale.x=s.wristYmm/2;
  wristMesh.scale.z=s.wristZmm/2;
  wristMesh.position.z=s.wristZmm/2;
  if(virtualAxis)virtualAxis.position.set(0,0,s.wristZmm/2);
  assembly.position.set(0,s.offsetYmm,s.offsetZmm);
  if(watchAnchor){
    watchAnchor.position.copy(watchBasePosition);
    if(s.watchState===1)watchAnchor.position.x-=24;
    watchAnchor.visible=s.watchState!==2;
  }
  return s;
}
...
function ensureUi(){
  ...
  if(!document.getElementById("engine4BankModes")){
    const state=document.createElement("div");state.id="engine4BankRigState";state.textContent="R1.5 · ESPERANDO FOTO";root.appendChild(state);
    const modes=document.createElement("div");modes.id="engine4BankModes";
    ["OCULTA","TRANSP.","SÓLIDA","OCLUSIÓN"].forEach((label,mode)=>{
      const b=document.createElement("button");b.type="button";b.dataset.mode=String(mode);b.textContent=label;b.classList.toggle("on",mode===wristMode);
      b.onclick=()=>{wristMode=mode;modes.querySelectorAll("button").forEach(x=>x.classList.toggle("on",Number(x.dataset.mode)===wristMode));applyAppearance();render();};
      modes.appendChild(b);
    });
...
async function loadModel(){
  ...
  modelPromise=(async()=>{
    ...
    watchModel=gltf.scene;watchModel.scale.setScalar(Number(modelConfig.scaleToMillimeters)||1000);watchModel.rotateX(Math.PI);
    wristRig=new Group();wristRig.name="AMURA_ENGINE4_BANK_RIG";
    assembly=new Group();assembly.name="AMURA_ENGINE4_BANK_ASSEMBLY";wristRig.add(assembly);
    watchAnchor=new Group();watchAnchor.name="AMURA_ENGINE4_BANK_WATCH_ANCHOR";watchAnchor.add(watchModel);assembly.add(watchAnchor);
    wristMaterial=new MeshBasicMaterial({color:0x8d6cff,transparent:true,opacity:.30,depthTest:true,depthWrite:false});
    wristMesh=new Mesh(new CapsuleGeometry(1,2,8,20),wristMaterial);wristMesh.name="AMURA_ENGINE4_BANK_WRIST";wristMesh.rotation.set(0,0,Math.PI/2);wristMesh.renderOrder=-1000;assembly.add(wristMesh);
    virtualAxis=new Mesh(new CylinderGeometry(1.15,1.15,WRIST_LENGTH_MM,12),new MeshBasicMaterial({color:0xc03cff,depthTest:false,depthWrite:false,transparent:true,opacity:.95}));
    virtualAxis.name="AMURA_ENGINE4_BANK_VIRTUAL_AXIS";virtualAxis.rotation.z=Math.PI/2;virtualAxis.renderOrder=100000;assembly.add(virtualAxis);
    scene.add(wristRig);
    wristRig.updateMatrixWorld(true);
    const contact=watchModel.getObjectByName(modelConfig.contactNode);
    if(contact){const w=new Vector3();contact.getWorldPosition(w);const contactOffset=watchAnchor.worldToLocal(w.clone());watchAnchor.position.copy(contactOffset).multiplyScalar(-1);}
    watchBasePosition.copy(watchAnchor.position);
    applyLabState();applyAppearance();draco.dispose();return watchModel;
  })().catch(e=>{console.error("R1.5 banco GLB",e);const state=document.getElementById("engine4BankRigState");if(state)state.textContent="R1.5 · ERROR GLB";throw e;});
  return modelPromise;
}
...
function ensureRenderer(rect){
  ...
  if(!renderer){
    scene=new Scene();camera=new PerspectiveCamera(50,1,1,20000);camera.position.set(0,0,0);camera.lookAt(0,0,-1);
    scene.add(new HemisphereLight(0xe8efff,0x24182f,2.25));scene.add(new AmbientLight(0xffffff,.85));
    const k=new DirectionalLight(0xffffff,3.2);k.position.set(-280,420,780);scene.add(k);
    const r=new DirectionalLight(0xa992ff,1.8);r.position.set(420,-160,520);scene.add(r);
    renderer=new WebGLRenderer({canvas:overlay,alpha:true,antialias:true,powerPreference:"high-performance",premultipliedAlpha:true});
    renderer.setPixelRatio(1);renderer.setClearColor(0x000000,0);renderer.outputColorSpace=SRGBColorSpace;renderer.toneMapping=ACESFilmicToneMapping;renderer.toneMappingExposure=1.18;loadModel().catch(()=>{});
  }
  const w=Math.max(1,Math.round(rect.width)),h=Math.max(1,Math.round(rect.height));renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix();
  overlay.style.left=`${rect.left}px`;overlay.style.top=`${rect.top}px`;overlay.style.width=`${rect.width}px`;overlay.style.height=`${rect.height}px`;return true;
}
function render(){if(renderer&&scene&&camera){applyLabState();applyAppearance();renderer.render(scene,camera);}}
...
function updateBankRig(){
  ...
  const rect=bank.getBoundingClientRect();if(rect.width<20||rect.height<20)return;
  overlay.style.display="block";if(!ensureRenderer(rect))return;
  const axis=readOrangeAxis(bank);if(!axis){if(wristRig)wristRig.visible=false;render();return;}
  const s=applyLabState(),widthPx=measureCloudWidth(axis),f=focalFor(bank.width,bank.height),depth=clamp(f*s.wristYmm/Math.max(1,widthPx),180,1200),anchor=projectP0(axis,target);
  ...
  const frame=bankFrameCache.get(target),frameReady=bankFrameCache.has(target),mpQ=frame?orientationFromBankFrame(axis,frame):null,state=document.getElementById("engine4BankRigState");
  if(!frameReady){
    if(wristRig)wristRig.visible=false;
    if(state)state.textContent=`R1.5 · ${target}° · CALCULANDO IMAGE 3D`;
    render();return;
  }
  ...
  if(wristRig){
    ...
    wristRig.quaternion.copy(orientation);wristRig.visible=true;
    lastGeometry={target,depth,widthPx,anchor,axis,orientationSource:mpQ?"IMAGE 3D":"fallback",wristYmm:s.wristYmm,wristZmm:s.wristZmm,offsetYmm:s.offsetYmm,offsetZmm:s.offsetZmm,watchState:s.watchState};
  }
  if(state)state.textContent=`R1.5 · ${target}° · ${mpQ?"IMAGE 3D = CÁMARA":"FALLBACK"} · Y ${s.wristYmm.toFixed(1)} · Z ${s.wristZmm.toFixed(1)}`;
  lastSignature=`${target}:${Math.round(axis.midpoint.x)}:${Math.round(axis.midpoint.y)}:${Math.round(axis.span)}:${Math.round(widthPx)}`;render();
}
function tick(){updateBankRig();}
function start(){ensureUi();timer=window.setInterval(tick,100);window.addEventListener("resize",tick);window.addEventListener("pagehide",()=>{if(timer)clearInterval(timer);},{once:true});}
start();
