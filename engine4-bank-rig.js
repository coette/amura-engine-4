...
const REVISION = "R18.1.0";
const MODEL_URL = "./models/A1-Irontide-AR-pretty-mobile.glb?v=r18.1.0";
const MODEL_CONFIG_URL = "./models/A1-Irontide-AR-pretty-mobile.json?v=r18.1.0";
...
function parseTarget(){const text=document.getElementById("r16BankPose")?.textContent||"";const m=text.match(/(0|30|60|90|135|150|165|180)\s*°/);return m?Number(m[1]):null;}
function bankActive(){return document.body.dataset.amuraMode==="bank" && !document.getElementById("r16Root")?.hidden;}
...
function makeOrientation(axis,target){
...
function ensureUi(){
  const root=document.getElementById("r16Root");if(!root)return false;
  if(!document.getElementById("r18BankStyle")){
    const s=document.createElement("style");s.id="r18BankStyle";s.textContent=`
      #r18BankThree{position:absolute;z-index:3;pointer-events:none;background:transparent}
      #r18BankModes{position:absolute;left:10px;right:10px;bottom:142px;z-index:8;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:6px}
      #r18BankModes button{min-height:42px;padding:0 4px;border:1px solid rgba(255,255,255,.38);border-radius:999px;background:rgba(5,10,17,.9);color:rgba(255,255,255,.74);font:800 10px/1 Arial,sans-serif}
      #r18BankModes button.on{background:rgba(0,133,164,.94);color:#fff;border-color:#fff}
      #r18BankRigState{position:absolute;left:10px;right:10px;bottom:188px;z-index:8;text-align:center;color:#fff;font:800 10px/1.2 Arial,sans-serif;text-shadow:0 1px 4px #000;pointer-events:none}
    `;document.head.appendChild(s);
  }
  if(!overlay){overlay=document.createElement("canvas");overlay.id="r18BankThree";root.appendChild(overlay);}
  if(!document.getElementById("r18BankModes")){
    const state=document.createElement("div");state.id="r18BankRigState";state.textContent="R18.1 · ESPERANDO FOTO";root.appendChild(state);
    const modes=document.createElement("div");modes.id="r18BankModes";
...
async function loadModel(){
  ...
  modelPromise=(async()=>{
    ...
    wristRig=new Group();watchAnchor=new Group();watchAnchor.add(watchModel);wristRig.add(watchAnchor);
  ...
  })().catch(e=>{console.error("R18.1 banco: GLB",e);document.getElementById("r18BankRigState")&&(document.getElementById("r18BankRigState").textContent="R18.1 · ERROR GLB");throw e;});
  return modelPromise;
}
...
function updateBankRig(){
  ...
  if(!ensureUi())return;
  const bank=document.getElementById("r16BankCanvas"),target=parseTarget();if(!bank||target===null)return;
  ...
  const sx=rect.width/bank.width,sy=rect.height/bank.height;
  ...
  if(wristRig){
    ...
    wristRig.quaternion.copy(makeOrientation(axis,target));wristRig.visible=true;lastGeometry={target,depth,widthPx,anchor,axis};
  }
  const state=document.getElementById("r18BankRigState");if(state)state.textContent=`R18.1 · ${target}° · Z≈${Math.round(depth)} mm · ancho ${Math.round(widthPx)} px`;
  lastSignature=sig;render();
}

function tick(){updateBankRig();}
function start(){ensureUi();timer=window.setInterval(tick,140);window.addEventListener("resize",tick);window.addEventListener("pagehide",()=>{if(timer)clearInterval(timer);},{once:true});}
start();

window.AmuraR18BankRig={revision:REVISION,get state(){return{active:bankActive(),wristMode,lastSignature,lastGeometry};}};
