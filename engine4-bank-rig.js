...
const REVISION = "ENGINE 4 · R1.1";
const MODEL_URL = "./models/A1-Irontide-AR-pretty-mobile.glb?v=e4r1.1";
const MODEL_CONFIG_URL = "./models/A1-Irontide-AR-pretty-mobile.json?v=e4r1.1";
...
function bankActive(){return document.body.dataset.amuraMode==="bank" && !document.getElementById("engine4BankRoot")?.hidden;}
...
function ensureUi(){
  ...
  if(!document.getElementById("engine4BankRigStyle")){
    const s=document.createElement("style");s.id="engine4BankRigStyle";s.textContent=`
      #engine4BankRigCanvas{position:absolute;z-index:3;pointer-events:none;background:transparent}
      #engine4BankRigModes{position:absolute;left:10px;right:10px;bottom:142px;z-index:8;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:6px}
      #engine4BankRigModes button{min-height:42px;padding:0 4px;border:1px solid rgba(255,255,255,.38);border-radius:999px;background:rgba(5,10,17,.9);color:rgba(255,255,255,.74);font:800 10px/1 Arial,sans-serif}
      #engine4BankRigModes button.on{background:rgba(0,133,164,.94);color:#fff;border-color:#fff}
  ...
  if(!overlay){overlay=document.createElement("canvas");overlay.id="engine4BankRigCanvas";root.appendChild(overlay);}
  if(!document.getElementById("engine4BankRigModes")){
    const state=document.createElement("div");state.id="engine4BankRigState";state.textContent="ENGINE 4 · R1.1 · ESPERANDO FOTO";root.appendChild(state);
    const modes=document.createElement("div");modes.id="engine4BankRigModes";
...
async function loadModel(){
  ...
  })().catch(e=>{console.error("ENGINE 4 R1.1 banco: GLB",e);document.getElementById("engine4BankRigState")&&(document.getElementById("engine4BankRigState").textContent="ENGINE 4 · R1.1 · ERROR GLB");throw e;});
  return modelPromise;
}
...
function updateBankRig(){
  ...
  const state=document.getElementById("engine4BankRigState");if(state)state.textContent=`ENGINE 4 · R1.1 · ${target}° · Z≈${Math.round(depth)} mm · ancho ${Math.round(widthPx)} px`;
