(function () {
  ...
  function hideInterface() {
    ...
    if (currentStatus === "live" && diagnosticsPanel.hidden) {
  ...
  function scheduleInterfaceHide() {
    ...
    if (currentStatus === "live" && diagnosticsPanel.hidden) {
      interfaceTimer = setTimeout(hideInterface, 2800);
    }
  }
  ...
  function requestAvailableFullscreen() {
    ...
    if (root.requestFullscreen) {
      root.requestFullscreen().catch(function () {
        /* iPhone Safari mantiene el control de sus barras. */
      });
    }
  }
  ...
  function setStatus(status, message) {
    ...
    if (status === "paused") {
      startButton.textContent = "Reanudar cámara";
    } else if (status === "error") {
      startButton.textContent = "Reintentar cámara";
    } else {
      startButton.textContent = "Abrir cámara trasera";
    }
    ...
    window.dispatchEvent(new CustomEvent("amura-camera-state", {
      detail: window.AmuraCameraState
    }));
  }
  ...
  function startFrameCounter() {
    ...
    function animationTick(now) {
      ...
      if (video.currentTime !== frameStats.lastVideoTime) {
        frameStats.lastVideoTime = video.currentTime;
        registerFrame(now);
      }
  ...
  function stopMedia(updateStatus) {
    ...
    if (updateStatus !== false) {
      setStatus("idle", "Cámara detenida. Puedes volver a abrirla cuando quieras.");
    }
  }
  ...
  function findCameraForFacing(facingMode) {
    ...
    var match = cameras.find(function (camera) {
  ...
  function attachTrackEvents(track) {
    track.addEventListener("ended", function () {
      cancelFrameCounter();
      setStatus("paused", "La cámara se ha detenido. Pulsa reanudar para abrirla de nuevo.");
    });

    track.addEventListener("mute", function () {
      setStatus("paused", "El sistema ha pausado temporalmente la cámara.");
    });

    track.addEventListener("unmute", function () {
      setStatus("live", "Cámara activa.");
      startFrameCounter();
    });
  }
  ...
  function startCamera(facingMode, deviceId) {
    ...
    stopMedia(false);
    setStatus("requesting", "Esperando permiso del navegador…");

    if (!window.isSecureContext || !navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      setStatus("error", "Este navegador no permite usar la cámara aquí. Abre el laboratorio desde la URL HTTPS de Cloudflare Pages.");
      return;
    }

    var request = navigator.mediaDevices.getUserMedia(
      preferredConstraints(requestedFacingMode, deviceId)
    );
    ...
    request.catch(function () {
      ...
      return video.play().then(function () {
        ...
        document.body.setAttribute("data-facing", currentFacingMode);
        if (currentDeviceId && facingDeviceIds[currentFacingMode] !== undefined) {
          facingDeviceIds[currentFacingMode] = currentDeviceId;
        }
      ...
      stopMedia(false);
      setStatus("error", cameraErrorMessage(error));
    });
  }
  ...
  function findActiveCameraLabel(track) {
    var active = cameras.find(function (camera) {
  ...
  function updateDiagnostics() {
    ...
    if (sourceWidth && sourceHeight && viewportWidth && viewportHeight) {
      ...
      var cropX = Math.max(0, (renderedWidth - viewportWidth) / 2);
      var cropY = Math.max(0, (renderedHeight - viewportHeight) / 2);
      crop = cropX.toFixed(0) + " × " + cropY.toFixed(0) + " px";
    }

    var data = [
      ["Versión", "Punto 5 · V11.2"],
      ["Estado", currentStatus],
      ["Interfaz debug", DEVELOPMENT_UI_ALWAYS_VISIBLE ? "fija" : "automática"],
      ["FPS medidos", measuredFps ? measuredFps.toFixed(1) : "—"],
      ["FPS cámara", typeof settings.frameRate === "number" ? settings.frameRate.toFixed(1) : "—"],
      ["Fuente vídeo", sourceWidth ? sourceWidth + " × " + sourceHeight : "—"],
      ["Ajuste track", settings.width ? settings.width + " × " + settings.height : "—"],
      ["Dirección", settings.facingMode || "no indicada"],
      ["Cámara", findActiveCameraLabel(track)],
      ["Track / mute", track ? track.readyState + " / " + track.muted : "—"],
      ["Cámaras vistas", String(cameras.length)],
      ["Viewport", viewportWidth + " × " + viewportHeight],
      ["DPR", window.devicePixelRatio.toFixed(2)],
      ["Orientación", orientationLabel()],
      ["Recorte cover", crop],
      ["HTTPS", window.isSecureContext ? "sí" : "no"],
      ["Visibilidad", document.visibilityState],
      ["Modo", isStandaloneMode() ? "web-app" : "navegador"],
      ["Fullscreen", document.fullscreenElement ? "sí" : "no"]
    ];

    var trackingData = window.AmuraTrackingDiagnostics || {};
    Object.keys(trackingData).forEach(function (key) {
      data.push([key, String(trackingData[key])]);
    });

    diagnosticsList.innerHTML = data.map(function (item) {
  ...
  function setDiagnosticsOpen(open) {
    ...
    diagnosticsButton.setAttribute("aria-pressed", open ? "true" : "false");
    if (open) {
      updateDiagnostics();
    } else {
  ...
  function handleVisibilityChange() {
    ...
    if (track && track.readyState === "ended") {
      setStatus("paused", "La cámara se cerró al salir del navegador. Pulsa reanudar.");
      return;
    }

    if (track && video.paused) {
      video.play().then(function () {
        setStatus("live", "Cámara activa.");
        startFrameCounter();
      }).catch(function () {
        setStatus("paused", "Pulsa reanudar para recuperar el vídeo.");
      });
    }
  }

  startButton.addEventListener("click", function () {
    requestAvailableFullscreen();
  ...
  diagnosticsButton.addEventListener("click", function () {
    setDiagnosticsOpen(diagnosticsPanel.hidden);
  });
  closeDiagnosticsButton.addEventListener("click", function () {
    setDiagnosticsOpen(false);
  });
  cameraLab.addEventListener("click", function () {
    if (currentStatus === "live") showInterface(true);
  });
  ...
  document.addEventListener("fullscreenchange", updateDiagnostics);
  ...
  window.addEventListener("pagehide", function (event) {
    if (!event.persisted) stopMedia(false);
  });
